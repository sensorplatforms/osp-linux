...
int OSPDaemon_get_sensor_data(int in_sen_type, struct psen_data *out_data)
{
	...
	DBG(DEBUG_INIT, "before checking q empty");
	DBG(DEBUG_INIT, "q->write %d q->read %d", sd->sensor[i].output->q.write,
	sd->sensor[i].output->q.read);
	/* Before returning check if there is more data and signal to indicate */
	if (0 == OSPDaemon_queue_isempty(&sd->sensor[i].output->q)) {
		DBG(DEBUG_INIT, "More data available posting sem");
	...
	DBG(DEBUG_INIT, "after checking q empty");
