...
static void OSPDaemon_enable(int sensorid, int enable){
	int fd;
	char buff[SYSFS_CMD_SIZE];
	int seq_no = 1, sz;
...
static int OSPDaemon_iio_disable(struct OSPDaemon_SensorDetail *s)
{
	if(hif_enable){
		int sensor_id = s->sensor.SensorType;
		OSPDaemon_enable(sensor_id, 0);
	}
	else {
	    struct IIO_Sensor *is;
	    is = s->lprivate;
		DBG(DEBUG_INIT,"%s ::: sensor_id %d\n", __func__, s->sensor.SensorType);
