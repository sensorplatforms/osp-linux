...
static int osp_i2c_write(u8 *data, int len)
{
	struct i2c_msg msgs[] = {
		{
			.addr = gOSP->client->addr,
...
static int osp_i2c_read_data(u8 *data, int len)
{
	struct i2c_msg msgs[] = {
		{
			.addr = gOSP->client->addr,
...
int process_sensor_data(u8 *buf, int len)
{
	...
	do {
		...
		pack_ptr += err;
		pack_count++;
		pr_info("OSP packet parsing success: bytes_parsed = %d,\
			pack_count = %d, plen = %d \n", err, pack_count, plen);
	} while (plen > 0);
...
static irqreturn_t osp_irq_thread(int irq, void *dev)
{
	...
	u16 cause = 0;
	u32 size = 0;
	int timeout = I2C_READ_MAX_RETRY;
	int ret = 0;
	cmd = cpu_to_be16(SENSORHUB_GET_CAUSE);
...
read_again:
	...
	pr_debug("%s :::\n", __func__);
	ret = osp_i2c_write(txbuf, sizeof(u32));
	if (ret < 0) {
		pr_debug("%s :: i2c write failed :: err : %d\n ", __func__, ret);
		return IRQ_HANDLED;
	}
	do {
		osp_i2c_read_data(rxbuf, sizeof(u16));
	} while (*((u16 *) rxbuf) != cmd && --timeout > 0);

	pr_debug("%s rxbuf=0x%04x, timeout=%d\n", __func__,\
			*((u16 *) rxbuf), timeout);

	if (*((u16 *) rxbuf) == cmd) {
		osp_i2c_read_data(rxbuf, sizeof(u16) * 3);
		cause = be16_to_cpu(*((u16 *)rxbuf));
		error_code = be16_to_cpu(*((u16 *)(rxbuf + sizeof(u16))));
		size = be16_to_cpu(*((u16 *)(rxbuf + sizeof(u32))));
		size = WORD_ALIGN(size, 4);
		pr_debug("%s: cause= 0x%04x error_code = 0x%04x size = 0x%08x\n",
			...
			size_t count)
{
	...
	osp_i2c_write(buff.buffer, buff.size);
			 ...
			 const struct i2c_device_id *id)
{
	...
	client->irq = gpio_to_irq(MPU_GYRO_IRQ_GPIO);
	#if 0
