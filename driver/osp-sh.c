...
read_again:
	...
	cause = *((u8 *)rxbuf);
	pr_debug("%s: cause= 0x%02x  ret = : %d", __func__, cause, ret);
	...
	{
		size = *(u16 *)(rxbuf + sizeof(u8));
		size = WORD_ALIGN(size, 2);
					...
					struct device_attribute *attr, char *buf)
{
	...
	pr_debug("%s ::\n", __func__);
	wait_for_completion(&hif_response_complete);
	if (gOSP->resp_data.seq_no == 0) {
		pr_debug("%s , seq_no : %d, response suppressed\n", __func__, \
			gOSP->resp_data.seq_no);
		return ret;
	}
	ret = snprintf(buf, HIF_RESP_DATA_SZ, "0x%02x %d 0x%02x %d",
		gOSP->resp_data.sensor_id, gOSP->resp_data.seq_no,
		gOSP->resp_data.param_id, error_code);
	return ret;
}
