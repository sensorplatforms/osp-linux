...
static struct mutex i2c_transaction;
static struct hif_write_data resp_data;
...
int format_write_response(char *buf, u8 paramid, u8 sensorid, u8 seqno,
	u8 dummy_data)
{
	int ret = 0;
	pr_debug("%s : response : %d\n", __func__, dummy_data);
	switch (paramid) {
	...
	case PARAM_ID_DYNAMIC_CAL_SOURCE:
	{
		...
		pr_debug("response is %s", buf);
		break;
	}
	default:
	{
		pr_debug("setconfig parameter is read only\n");
		ret = snprintf(buf, HIF_RESP_DATA_SZ, "0x%02x %d 0x%02x %d",
		sensorid, seqno, paramid, error_code);
		break;
	}
	...
	u8 dummy_data)
{
	...
	case PARAM_ID_REPEATABILITY:
	{
		if (dummy_data) {
			...
			ret = snprintf(buf, HIF_RESP_DATA_SZ, "0x%02x %d 0x%02x %d "
					"0x%08x 0x%08x 0x%08x", sensorid, seqno,
					paramid, error_code, data[0], data[1],
			data[2]);
		}
		else {
			/*Need to add read response data*/
			...
			size_t count)
{
	...
	mutex_lock(&i2c_transaction);
	buff = osp_read_packet(param_id, sensor_id, seq_no);
	/*write Hif read request packet to firmware*/
	//osp_i2c_write(OSP_DATA_OUT, &buff.buffer, buff.size);
	mutex_unlock(&i2c_transaction);
	return count;
}
			 ...
			 size_t count)
 {
	...
	resp_data.sensor_id = sensor_id;
	resp_data.seq_no = seq_no;
	...
	case PARAM_ID_ENABLE:
	{
		...
		pr_debug("%s param id : %d sensor id : 0x%02x, seq_no : %d,\
			...
			enable);
		mutex_lock(&i2c_transaction);
		buff = osp_sensor_enable(param_id, sensor_id, seq_no, enable);
		mutex_unlock(&i2c_transaction);
		break;
	}
	case PARAM_ID_BATCH:
	{
		...
		mutex_lock(&i2c_transaction);
		buff = osp_set_batch(param_id, sensor_id, seq_no, data);
		mutex_unlock(&i2c_transaction);
		break;
	}
	case PARAM_ID_FLUSH:
	{
		pr_debug("%s param id : %d sensor id : 0x%02x, seq_no : %d,\
				\n", __func__, param_id, sensor_id, seq_no);
		mutex_lock(&i2c_transaction);
		buff = osp_set_flush(param_id, sensor_id, seq_no);
		mutex_unlock(&i2c_transaction);
		pr_debug("%s param_id = %d sensor id : 0x%02x, seq_no : %d\n",\
			__func__, param_id, sensor_id, seq_no);
		break;
	...
	case PARAM_ID_AXIS_MAPPING:
	{
		...
		mutex_lock(&i2c_transaction);
		buff = osp_set_axis_mapping(param_id, sensor_id, seq_no,
			data);
		mutex_unlock(&i2c_transaction);
		break;
	}
	case PARAM_ID_CONVERSION_OFFSET:
	{
		...
		pr_debug("%s param id : %d sensor id : 0x%02x, seq_no : %d,\
			data[0] = %d, data[1] = %d, data[2] = %d \n",\
			 __func__, param_id, sensor_id, seq_no, data[0], data[1], data[2]);
		mutex_lock(&i2c_transaction);
		buff = osp_set_conversion_offset(param_id, sensor_id, seq_no,
			data);
		mutex_unlock(&i2c_transaction);
		break;
	}
	...
	case PARAM_ID_EXPECTED_NORM:
	{
		...
		mutex_lock(&i2c_transaction);
		buff = osp_set_time_offset_normal_mag(param_id, sensor_id,
		seq_no, data);
		mutex_unlock(&i2c_transaction);
		break;
	}
	case PARAM_ID_ONTIME_WAKETIME:
	{
		...
		pr_debug("%s param id : %d sensor id : 0x%02x, seq_no : %d, \
				...
				param_id, sensor_id, seq_no, data[0], data[1]);
		mutex_lock(&i2c_transaction);
		buff = osp_set_on_wake_time(param_id, sensor_id, seq_no, data);
		mutex_unlock(&i2c_transaction);
		break;
	}
	case PARAM_ID_HPF_LPF_CUTOFF:
	{
		...
		pr_debug("%s param id : %d sensor id : 0x%02x, seq_no : %d, \
				...
				param_id, sensor_id, seq_no, data[0], data[1]);
		mutex_lock(&i2c_transaction);
		buff = osp_set_hpf_lpf_cutoff(param_id, sensor_id, seq_no,
						data);
		mutex_unlock(&i2c_transaction);
		break;
	}
	case PARAM_ID_F_SKOR_MATRIX:
	{
		...
		pr_debug("%s param id : %d sensor id : 0x%02x, seq_no : %d,\
				\n", __func__, param_id, sensor_id, seq_no);
		mutex_lock(&i2c_transaction);
		buff = osp_set_skor_matrix(param_id, sensor_id, seq_no, data);
		mutex_unlock(&i2c_transaction);
		break;
	}
	case PARAM_ID_F_NONLINEAR_EFFECTS:
	{
		...
		pr_debug("%s param id : %d sensor id : 0x%02x, seq_no : %d,\
				\n", __func__, param_id, sensor_id, seq_no);
		mutex_lock(&i2c_transaction);
		buff = osp_set_non_linear_effects(param_id, sensor_id, seq_no,
		data);
		mutex_unlock(&i2c_transaction);
		break;
	}
	case PARAM_ID_TEMP_COEFF:
	{
		...
		pr_debug("%s param id : %d sensor id : 0x%02x, seq_no : %d,\
				\n", __func__, param_id, sensor_id, seq_no);
		mutex_lock(&i2c_transaction);
		buff = osp_set_temp_coeff(param_id, sensor_id, seq_no, data);
		mutex_unlock(&i2c_transaction);
		break;
	}
	case PARAM_ID_SHAKE_SUSCEPTIBILIY:
	{
		...
		pr_debug("%s param id : %d sensor id : 0x%02x, seq_no : %d, \
			...
			data[1], data[2]);
		mutex_lock(&i2c_transaction);
		buff = osp_set_shake_suscept(param_id, sensor_id, seq_no,
					data);
		mutex_unlock(&i2c_transaction);
		break;
	}
	...
	case PARAM_ID_REPEATABILITY:
	{
		...
		mutex_lock(&i2c_transaction);
		buff = osp_set_calib_param_fix32(param_id, sensor_id,
		seq_no, data);
		mutex_unlock(&i2c_transaction);
		break;
	}
	case PARAM_ID_DYNAMIC_CAL_SOURCE:
	{
		...
		mutex_lock(&i2c_transaction);
		buff = osp_set_dynamic_calib_source(param_id, sensor_id, seq_no, data);
		mutex_unlock(&i2c_transaction);
				     ...
				     struct device_attribute *attr, char *buf)
{
			 ...
			 size_t count)
 {
	 ...
	 memset(dummy_read_data, 0, sizeof(buffer));
	 sscanf(buf, "0x%02x %d 0x%02x", &sensor_id, &seq_no, &param_id);
			 ...
			 size_t count)
 {
	 int sensor_id, seq_no, param_id;
	 u8 buffer[4];
	 memset(dummy_write_data, 0, sizeof(buffer));
	 memset(buffer, 0, sizeof(buffer));
	 sscanf(buf, "0x%02x %d 0x%02x", &sensor_id, &seq_no, &param_id);
	 pr_debug("%s : dummy param id : %d sensor id : %d seq no : %d\n",
		 __func__, param_id, sensor_id, seq_no);
					  ...
					  struct device_attribute *attr, char *buf)
 {
	 ...
	 ret = format_write_response(buf, param_id, sensor_id, seq_no,
		 SENSOR_DUMMY_RESPONSE);
	 pr_debug("%s : %d\n", __func__, ret);
	 return ret;
 }
