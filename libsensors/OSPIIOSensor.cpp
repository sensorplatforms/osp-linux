...
#include "OSPIIOSensor.h"

OSPIIOSensor::OSPIIOSensor(const char* iioname,
				int32_t sensorId,
				int32_t sensorType,
				bool evtFloat) :
	SensorBase(NULL, iioname, 1),    
	mEnabled(false),
...
int OSPIIOSensor::setInitialState()
{
#if 0
...
int OSPIIOSensor::enable(int32_t handle, int enabled)
{
...
bool OSPIIOSensor::handleEvent(input_event const * event,
				sensors_event_t* androidData)
{
	return false;
}

bool OSPIIOSensor::hasPendingEvents() const
{
	return mHasPendingEvent;
}

int OSPIIOSensor::setDelay(int32_t handle, int64_t delay_ns)
{
	LOGI("@@@@ setDelay: [%d] - %lldms", handle, (delay_ns/1000000));
	return 0;
}
#undef USE_EMBEDDED_TS
int OSPIIOSensor::readEvents(sensors_event_t* data, int count)
{
	int Qscale = 1;
	if (count < 1)
		return -EINVAL;
